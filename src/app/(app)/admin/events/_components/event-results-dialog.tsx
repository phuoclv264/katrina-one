...
import { getEventVotes, getEventDraws, deleteVote, runPrizeDraw, subscribeToBallotDraws } from '@/lib/events-store';
...
export default function EventResultsDialog({ isOpen, onClose, event, allUsers, parentDialogTag }: EventResultsDialogProps) {
    ...
    useEffect(() => {
        if (!isOpen || event.type !== 'ballot') return;
        const unsubscribe = subscribeToBallotDraws(event.id, (liveDraws) => {
            setDraws(liveDraws);
            setLatestDraw(liveDraws[0] || null);
        });
        return unsubscribe;
