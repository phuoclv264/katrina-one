...
export default function IncidentCategoryDialog({ open, onOpenChange, parentDialogTag }: { open: boolean, onOpenChange: (open: boolean) => void, parentDialogTag: string }) {
  ...
  useEffect(() => {
    if (open) {
      setIsLoading(true);
      const unsub = dataStore.subscribeToIncidentCategories((cats) => {
        setCategories(cats);
        setIsLoading(false);
      });
      return () => unsub();
  ...
  const handleSave = async (newCategoryList: IncidentCategory[]) => {
    try {
      await dataStore.updateIncidentCategories(newCategoryList);
  ...
  const handleAdd = () => {
    ...
    const newCategory: IncidentCategory = { id: uuidv4(), name: newCategoryName.trim() };
