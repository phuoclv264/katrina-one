...
export default function ChecklistTabs({ shift, report, otherStaffReports, activeTab, setActiveTab, expandedTaskIds, toggleExpandTask, handleBooleanTaskAction, handlePhotoTaskAction, handleOpinionTaskAction, handleNoteTaskAction, handleDeletePhoto, handleDeleteCompletion, onOpenLightbox, isReadonly, isSubmitting }: Props) {
  ...
  const renderCompletionIndicator = (taskId: string, sectionTitle: string) => {
    ...
    const selfCount = (report.completedTasks[taskId]?.length || 0);
    ...
    return (
      <>
        ...
        {/* Personal Status (Bottom-Right): Your progress towards the requirement */}
        {selfCount > 0 && (
          <div className="absolute -bottom-1.5 -right-1.5 z-20 pointer-events-none">
            {selfCount >= min ? (
              <div className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-green-500 text-white shadow-md border-2 border-white animate-in zoom-in-50 duration-300">
                <CheckCircle className="w-4 h-4" />
              </div>
            ) : (
              <div className="inline-flex items-center justify-center h-6 min-w-[32px] px-1.5 rounded-full bg-white text-slate-600 text-[10px] font-black shadow-sm border-2 border-slate-200 animate-in zoom-in-50 duration-300">
                {selfCount}/{min}
              </div>
            )}
          </div>
