    ...
    if (filteredShift && report && !isReadonly) {
      ...
      for (const section of sections) {
        ...
        const isSectionComplete = sectionTasks.length > 0 && sectionTasks.every(t => {
          ...
          const minCompletions = calculateAdjustedMinCompletions(baseReq, shiftKey, activeShifts?.[0]?.timeSlot);
          
          if (t.isTeamJob) {
            // Check if ANYONE (including me) completed this team task
            const doneByAnyone = [report, ...otherStaffReports].some(r => (r.completedTasks?.[t.id]?.length || 0) >= minCompletions);
            return doneByAnyone;
          }
          // Self-completion only for individual tasks
          return (report.completedTasks[t.id]?.length || 0) >= minCompletions;
        });
