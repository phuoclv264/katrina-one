...
export const ViolationCard = React.forwardRef<HTMLDivElement, ViolationCardProps>(({
    ...
    const cardBorderColor = v.isFlagged ? 'violation-high-border violation-high-ring' : (isWaived ? 'violation-low-border violation-low-ring' : getSeverityBorderClass(v.severity));
    const cardBgColor = v.isFlagged ? 'violation-high-bg' : (isWaived ? 'violation-low-bg' : getSeverityCardClass(v.severity));
        ...
        return (
            <Card ref={ref} className={cn("relative shadow-sm hover:shadow-lg transition-shadow bg-card", cardBorderColor, cardBgColor)}>
                <CardContent className="p-4">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 flex-wrap mt-1">
                                ...
                                {v.users.length === 1 && v.users[0].id === v.reporterId && (
                                    <Badge variant="outline" className="violation-low">Tự thú</Badge>
                                )}
                            </div>
                        </div>
                        {/* Right side: Actions */}
                            ...
                            {isOwner && (
                                <>
                                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onToggleWaivePenalty(v)} disabled={isItemProcessing}>
                                        <Flag className={cn("h-4 w-4", isWaived ? "violation-low" : "violation-low")} />
                                    </Button>
                                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onToggleFlag(v)} disabled={isItemProcessing}>
                                        <Flag className={cn("h-4 w-4", v.isFlagged ? "violation-high" : "violation-high")} />
                                    </Button>
                    ...
                    <p className={cn("mt-2 font-bold text-lg", isWaived ? "violation-low line-through" : "status-error")}>Tổng phạt: {(v.cost || 0).toLocaleString('vi-VN')}đ
                        {v.users.length > 1 && (
                    ...
                    <div className="mt-4 pt-4 border-t space-y-4">
                        {v.users.map((violatedUser) => {
                            ...
                            if (isWaived) {
                                return (
                                    <div key={violatedUser.id} className="text-sm violation-low font-semibold flex items-center gap-2">
                            ...
                            if (submission) {
                                ...
                                return (
                                    <div key={violatedUser.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                        <div className="text-sm violation-low font-semibold flex items-center gap-2">
                                            <CheckCircle className="h-4 w-4" />
