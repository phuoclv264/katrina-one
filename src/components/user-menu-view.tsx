...
export default function UserMenuView({ onNavigateToHome, onNavigate }: UserMenuViewProps) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        if (Capacitor.isNativePlatform()) {
          ...
          if (!mounted) return;
          setActiveVersion(snap.activeVersion ?? packageJson.version ?? 'builtin');
          setLatestVersion(snap.lastManifestVersion ?? null);
        } else {
          setActiveVersion(packageJson.version ?? null);
          setLatestVersion(null);
        }
      } catch (err) {
        if (mounted) setActiveVersion(packageJson.version ?? null);
      }
    };
    ...
    if (Capacitor.isNativePlatform()) {
      App.addListener('resume', () => { void refresh(); }).then(l => { resumeListener = l; }).catch(() => {});
    }
      ...
      <div className="flex-1 px-4 pb-6">
        ...
        <div className="mt-4 mb-4 p-4 border-t bg-background/50">
          ...
          <div className="mt-3 text-xs text-muted-foreground space-y-1">
            <div>Phiên bản hiện tại: <span className="font-medium">{activeVersion ?? '—'}</span></div>
            <div>Bản mới nhất: <span className="font-medium">{latestVersion ?? '—'}</span></div>
          </div>
        </div>
      </div>
    </div>
