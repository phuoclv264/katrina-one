...
/**
 * Live subscription to ballot draws so UI updates immediately at draw time.
 */
export function subscribeToBallotDraws(
    eventId: string,
    callback: (draws: PrizeDrawResult[], snapshot: QuerySnapshot) => void,
): () => void {
    const drawsQuery = query(collection(db, `events/${eventId}/draws`), orderBy('drawnAt', 'desc'));
    return onSnapshot(drawsQuery, (snap) => {
        const draws = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as PrizeDrawResult));
        callback(draws, snap);
    }, (error) => {
        console.error('subscribeToBallotDraws error:', error);
    });
}
